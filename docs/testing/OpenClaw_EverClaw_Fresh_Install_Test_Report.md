...
2. Session exited, `openclaw gateway restart` performed
3. On restart, the model **remained** `mor-gateway/kimi-k2.5`
4. A new session via `/new` also inherited the setting
...
2. Default `mor-gateway/glm-5` **works** — the previously reported "too expensive" error did not reproduce.
3. Skill directory is **`everclaw`**.
4. **New blocking bug:** the `bonjour` plugin block fails config validation on fresh install; the gateway will not start until it is removed.
5. Bootstrap provisions only three models; `llama-3.3-70b` needs an alias entry, `deepseek-v4-flash` needs a full two-part manual add.
6. Proxy-router auto-install fails but is genuinely non-fatal for Gateway use.
7. Several `diagnose.sh` failures are expected in Gateway-only mode and require interpretation guidance.
